...
export default function CorporationPage() {
  ...
  const [membershipStatus, setMembershipStatus] = useState(null) // null | 'pending' | 'approved' | 'declined'
  const [loading,          setLoading]          = useState(true)
  const [notFound,         setNotFound]         = useState(false)
  const [tab,              setTab]              = useState('about')

  useEffect(() => { fetchCorp() }, [slug])
  useEffect(() => { if (user && corp) checkMembership() }, [user, corp?.id])
  ...
  const checkMembership = async () => {
    ...
    const { data } = await supabase
      ...
      .maybeSingle()
    setMembershipStatus(data?.request_status ?? null)
  }
  ...
  const MembershipBadge = () => {
    if (!user) return (
      <Link to="/register" className="btn-primary text-sm">Join as volunteer</Link>
    )
    if (membershipStatus === 'approved') return (
      <span className="badge bg-brand-50 text-brand-700 border border-brand-200 text-sm px-3 py-1.5">✓ Member</span>
    )
    if (membershipStatus === 'pending') return (
      <span className="badge bg-amber-50 text-amber-700 border border-amber-200 text-sm px-3 py-1.5">⏳ Pending approval</span>
    )
    if (membershipStatus === 'declined') return (
      <span className="badge bg-red-50 text-red-700 border border-red-200 text-sm px-3 py-1.5">Membership declined</span>
    )
    return null
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="card mb-6">
        <div className="flex flex-col sm:flex-row items-start gap-5">
          <div className="w-20 h-20 rounded-2xl bg-amber-50 flex items-center justify-center text-amber-600 font-semibold text-2xl shrink-0 overflow-hidden border border-gray-100">
            {corp.logo_url
              ? <img src={corp.logo_url} alt={name} className="w-full h-full object-cover" />
              : initials}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-gray-400">
              ...
              <span className="flex items-center gap-1">
                ...
                {memberCount} enrolled member{memberCount !== 1 ? 's' : ''}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-2 shrink-0 flex-wrap">
            {corp.website && (
              <a href={corp.website} target="_blank" rel="noreferrer" className="btn-secondary text-sm">Website ↗</a>
            )}
            <MembershipBadge />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {(corp.facebook_url || corp.instagram_url || corp.linkedin_url) && (
              <div className="card">
                ...
                <div className="flex flex-col gap-3">
                  <SocialLink href={corp.facebook_url} label="Facebook"
                  ...
                  />
                  <SocialLink href={corp.instagram_url} label="Instagram"
                  ...
                  />
                  <SocialLink href={corp.linkedin_url} label="LinkedIn"
          ...
          <div className="flex flex-col gap-4">
            <div className="card">
              ...
              <InfoRow label="Founded"        value={corp.founded_year?.toString()} />
            ...
            {!user && (
              <div className="card bg-amber-50 border-amber-100 text-center">
                ...
                <p className="text-xs text-amber-600 mb-3 leading-relaxed">
                  Register as a corporate volunteer to join this company's CSR program.
                </p>
      ...
      {/* MEMBERS TAB — intentionally private */}
      {tab === 'members' && (
