...
const VISIBILITY_OPTIONS = [
  ...
  { value: 'registered', label: 'Registered users', desc: 'Visible only to logged-in users',                    icon: '🔒' },
...
function FieldEditor({ field, value, visibility, isPublished, onSave }) {
  ...
  const currentVis = VISIBILITY_OPTIONS.find(o => o.value === vis)
    ...
    <div className={`border rounded-xl transition-all ${open ? 'border-brand-200 shadow-sm' : 'border-gray-100'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              {currentVis?.icon} {currentVis?.label} · {pub ? 'Published' : 'Draft'}
      ...
      {open && (
        <div className="px-4 pb-4 flex flex-col gap-3 border-t border-gray-100 pt-3">
          {field.type === 'textarea'
            ? <textarea rows={4} className="input resize-none text-sm" value={val}
                onChange={e => setVal(e.target.value)} placeholder={`Enter ${field.label.toLowerCase()}…`} />
            : <input type={field.type} className="input text-sm" value={val}
                onChange={e => setVal(e.target.value)} placeholder={`Enter ${field.label.toLowerCase()}…`} />
          }
              ...
              {VISIBILITY_OPTIONS.map(opt => (
                <label key={opt.value}
                  className={`flex items-start gap-3 p-2.5 rounded-lg border cursor-pointer transition-colors
                    ${vis === opt.value ? 'border-brand-300 bg-brand-50' : 'border-gray-100 hover:border-gray-200'}`}>
                  <input type="radio" name={`vis-${field.key}`} value={opt.value}
                    checked={vis === opt.value} onChange={() => setVis(opt.value)} className="mt-0.5" />
                  <div>
          ...
          <div className="flex items-center justify-between py-2 border-t border-gray-100">
            <div>
              ...
              <p className="text-xs text-gray-400">When off, this field is hidden from the portal</p>
            </div>
            <button type="button" onClick={() => setPub(p => !p)}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${pub ? 'bg-brand-400' : 'bg-gray-200'}`}>
          ...
          <button onClick={handleSave} disabled={saving || !dirty}
            className={`btn-primary text-sm flex items-center justify-center gap-2 ${(!dirty && !saving) ? 'opacity-50 cursor-not-allowed' : ''}`}>
...
export default function CorpContentEditor() {
  ...
  const load = async () => {
    ...
    const [{ data: corpData }, { data: contentRows }] = await Promise.all([
      supabase.from('corporations').select('*').eq('id', memberRow.corporation_id).single(),
      supabase.from('corp_content').select('*').eq('corporation_id', memberRow.corporation_id),
    ])

    setCorp(corpData)
    const map = {}
    ;(contentRows || []).forEach(r => { map[r.field_name] = r })
    setContent(map)
    setLoading(false)
  }
  ...
  const handleSave = async (fieldName, value, visibility, isPublished) => {
    if (!corp) return
    await supabase.from('corp_content').upsert({
    ...
    }, { onConflict: 'corporation_id,field_name' })
      ...
      <div className="flex flex-col gap-3">
        {FIELDS.map(field => (
          <FieldEditor
            key={field.key}
            field={field}
            value={content[field.key]?.field_value || ''}
            visibility={content[field.key]?.visibility || 'public'}
            isPublished={content[field.key]?.is_published ?? false}
            onSave={handleSave}
          />
        ))}
      </div>
