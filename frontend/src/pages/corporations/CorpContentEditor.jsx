...
function FieldEditor({ field, value, visibility, isPublished, onSave }) {
  ...
  const currentVis = VIS_OPTIONS.find(o => o.value === vis)
  ...
  const headerClass = 'border rounded-xl transition-all ' + (open ? 'border-brand-200 shadow-sm' : 'border-gray-100')
  const tabClass = (active) => 'px-4 py-2.5 text-sm font-medium border-b-2 transition-colors -mb-px ' + (active ? 'border-brand-400 text-brand-600' : 'border-transparent text-gray-500 hover:text-gray-700')
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              {currentVis && currentVis.icon + ' ' + currentVis.label} · {pub ? 'Published' : 'Draft'}
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {VIS_OPTIONS.map(opt => (
                <label
                  key={opt.value}
                  className={'flex items-start gap-3 p-2.5 rounded-lg border cursor-pointer transition-colors ' + (vis === opt.value ? 'border-brand-300 bg-brand-50' : 'border-gray-100 hover:border-gray-200')}
                >
...
export default function CorpContentEditor() {
  ...
  const load = async () => {
    ...
    setCorp(corpData)
    const map = {}
    if (contentRows) contentRows.forEach(r => { map[r.field_name] = r })
    setContent(map)
    setLoading(false)
  }
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      {corp.status === 'pending' && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6 text-sm text-amber-700">
          Pending approval. Content changes are saved but not visible until approved.
        </div>
      )}
      ...
      <div className="flex flex-col gap-3">
        {FIELDS.map(field => (
          <FieldEditor
            key={field.key}
            field={field}
            value={(content[field.key] && content[field.key].field_value) || ''}
            visibility={(content[field.key] && content[field.key].visibility) || 'public'}
            isPublished={(content[field.key] && content[field.key].is_published) || false}
            onSave={handleSave}
          />
        ))}
      </div>
