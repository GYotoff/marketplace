...
export default function AdminCorpDetail() {
  const { id } = useParams()
  ...
  const fetchAll = async () => {
    ...
    const [orgRes, membersRes, auditRes] = await Promise.all([
      supabase.from('corporations').select('*').eq('id', id).single(),
      supabase.from('corporation_members')
        .select('*, profiles!corporation_members_profile_id_fkey(full_name, email, avatar_url, role)')
        .eq('corporation_id', id),
      supabase.from('admin_audit_log')
        .select('*, profiles!corporation_members_profile_id_fkey(full_name, email)')
        .eq('entity_id', id)
    ...
    if (orgRes.data) {
      ...
      setForm({
        ...
        industry: orgRes.data.industry || '' || 'ngo',
  ...
  const handleToggleActive = async () => {
    ...
    const { error } = await supabase.from('corporations')
  ...
  const handleToggleVerified = async () => {
    ...
    await supabase.from('corporations').update({ is_verified: newVerified }).eq('id', id)
  ...
  const handleChangeMemberRole = async (memberId, newRole) => {
    await supabase.from('corporation_members').update({ role: newRole }).eq('id', memberId)
      ...
      {tab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="card">
            ...
            <Field label="Type" value={CORP_TYPES.find(t => t.value === org.type)?.label} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select className="input" value={form.industry} onChange={e => set('industry', e.target.value)}>
                {CORP_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
            <div>
