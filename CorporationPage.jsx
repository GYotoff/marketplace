...
export default function CorporationPage() {
  ...
  useEffect(() => { if (user && corp) checkMembership() }, [user, corp && corp.id])
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[60vh]"><div className="w-8 h-8 border-2 border-brand-400 border-t-transparent rounded-full animate-spin" /></div>

  if (notFound) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
      <p className="text-4xl mb-4">🏢</p>
      <h1 className="text-xl font-medium text-gray-900 mb-2">Corporation not found</h1>
      <p className="text-gray-500 text-sm mb-6">This corporation doesn't exist or is not yet approved.</p>
  ...
  const TABS = [{ key: 'about', label: 'About' }, { key: 'members', label: 'Members (' + memberCount + ')' }]

  const MemberBadge = () => {
    if (!user) return <Link to="/register" className="btn-primary text-sm">Join as volunteer</Link>
    if (memberStatus === 'approved') return <span className="badge bg-brand-50 text-brand-700 border border-brand-200 text-sm px-3 py-1.5">✓ Member</span>
    if (memberStatus === 'pending') return <span className="badge bg-amber-50 text-amber-700 border border-amber-200 text-sm px-3 py-1.5">⏳ Pending approval</span>
    if (memberStatus === 'declined') return <span className="badge bg-red-50 text-red-700 border border-red-200 text-sm px-3 py-1.5">Membership declined</span>
    return null
  }
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="card mb-6">
        <div className="flex flex-col sm:flex-row items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-gray-400">
              {corp.city && <span className="flex items-center gap-1"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>{corp.city}</span>}
              {corp.founded_year && <span>Est. {corp.founded_year}</span>}
              <span>{memberCount} enrolled member{memberCount !== 1 ? 's' : ''}</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-5 pt-5 border-t border-gray-100">
          {[{ label: 'Enrolled members', value: memberCount }, { label: 'Volunteer hours', value: 0 }, { label: 'Projects joined', value: 0 }].map(s => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {(corp.facebook_url || corp.instagram_url || corp.linkedin_url) && (
              <div className="card">
                ...
                <div className="flex flex-col gap-3">
                  <SocialLink href={corp.facebook_url} label="Facebook" icon={<svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>} />
                  <SocialLink href={corp.instagram_url} label="Instagram" icon={<svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>} />
                  <SocialLink href={corp.linkedin_url} label="LinkedIn" icon={<svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>} />
                </div>
              </div>
            )}
          </div>
          <div className="flex flex-col gap-4">
            <div className="card">
              ...
              <InfoRow label="Founded" value={corp.founded_year && corp.founded_year.toString()} />
