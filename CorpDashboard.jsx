...
export default function CorpDashboard() {
  ...
  const fetchAll = async () => {
    ...
    const [{ data: corpData }, { data: membersData }] = await Promise.all([
  ...
  const handleAction = async (memberId, action) => {
    ...
    const { error } = await supabase.from('corporation_members')
      .update({ request_status: status, reviewed_by: user.id, reviewed_at: new Date().toISOString() })
      .eq('id', memberId)
    if (!error) { showToast('Done'); fetchAll() }
  ...
  const STATUS_COLOR = { pending: 'text-amber-600', approved: 'text-brand-600', declined: 'text-red-600', suspended: 'text-gray-500' }
      ...
      <div className="card mb-6 flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className={'text-sm font-medium capitalize ' + (STATUS_COLOR[corp.status] || 'text-gray-500')}>{corp.status}</p>
      ...
      {tab === 'members' && (
        <div className="flex flex-col gap-3">
          {approved.length === 0 ? (
            ...
            const initials = m.profiles?.full_name?.[0] || m.profiles?.email?.[0] || '?'
            return (
              <div key={m.id} className="card flex items-center gap-4 flex-wrap">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900">
                    {m.profiles?.full_name || '—'}{m.profile_id === user.id && <span className="text-xs text-gray-400 ml-1">(you)</span>}
                  </p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {m.profile_id !== user.id && (
                    <button onClick={() => handleRemove(m.id, m.profiles?.full_name || 'member')}
        ...
        <div className="flex flex-col gap-3">
          {pending.length === 0 ? (
            ...
            <div key={m.id} className="card flex items-start gap-4 flex-wrap">
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => handleAction(m.id, 'approve')} disabled={actionLoading === m.id}
                  className="btn-primary text-xs py-1.5">
                  {actionLoading === m.id ? '...' : '✓ Approve'}
                </button>
                <button onClick={() => handleAction(m.id, 'decline')} disabled={actionLoading === m.id}
